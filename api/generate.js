...
function splitTitleAndBody(s) {
  ...
  if (parts.length === 2) {
    ...
    let body = (parts[1] ?? s).trim();
    if (!body) body = s.trim(); // ★ 本文が空になった場合の安全化
    return { title, body };
  }
...
// ▼ 修正：GENERAL_DEFS(k) → GENERAL_DEFS[k]
...
const prompt = [
"これは健全で安全な舞台用の漫才台本です。",
"必ず【タイトル】と本文（台本）を出力し、空出力はしないでください。",
...
// ★ フォールバック：空出力のまま 500 になりそうなら一度だけ再生成して回避
let success = typeof body === "string" && body.trim().length > 0;  
if (!success) {
  try {
    const rescueMessages = [
      { role: "system", content: "あなたは実力派の漫才師コンビです。安全で健全な台本のみを出力してください。必ず【タイトル】と本文を出力してください。" },
      { role: "user", content: "200〜400文字の短い漫才台本を1本、【タイトル】と本文（会話形式）で出力してください。" }
    ];
    const rescue = await client.chat.completions.create({
      model: process.env.OPENAI_MODEL || "gpt-5",
      messages: rescueMessages,
      max_completion_tokens: 600
    });
    const rescueRaw = rescue?.choices?.[0]?.message?.content?.trim() || "";
    const pick = splitTitleAndBody(rescueRaw);
    title = title || pick.title || "（タイトル未設定）";
    body = enforceCharLimit(pick.body, Math.max(120, minLen), maxLen, false);
    body = normalizeSpeakerColons(body);
    body = ensureBlankLineBetweenTurns(body);
    success = typeof body === "string" && body.trim().length > 0;
  } catch (e) {
    console.warn("[rescue] failed:", e?.message || e);
  }
}
